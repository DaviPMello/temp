...
int main(){
    ...
    while (!end){
        ...
        if(menu.state == 0){
            ...
            if((score != 0) && (score % 200 == 100) && (!truck.active)){
                truck.active = true;
            ...
            if(plane.active){
                if(plane.defineVars){
                    plane.x = -1200;
                    plane.y = (screen.h/2) - (plane.pos.h/2) * screen.hScale;
                    plane.speed.x = 50;
                    plane.endCounter = 2800;
                    plane.attacks = false;
            ...
            ///Initial animation:
            //Animação inicial:
            if(plane.speed.x > 0 && plane.endCounter > 2700 && plane.endCounter < 2800 && car.health > 0){
                plane.speed.x--;
            } else if(plane.speed.x < 60 && plane.endCounter > 2600 && plane.endCounter <= 2700 && car.health > 0){
                plane.speed.x++;
            } else {
                plane.attacks = true;
            }
            ...
            if(plane.endCounter % 100 == 0 && plane.endCounter < 2600){
                if(plane.x > 0){
            ...
            if(plane.attacks){
                if((plane.x + (plane.pos.w/2)) > car.x - 40 && (plane.x + (plane.pos.w/2)) < car.x + 40){
                    ...
                    bomb.spritePos = {bomb.x, bomb.y, 145, 95};
                    bomb.counter = 60;
                }
            }
            bomb.counter--;
            cout << bomb.counter << ", " << bomb.spritePos.h << ", " <<  bomb.spritePos.w << "\n";
            if(bomb.counter > 20){
                bomb.spritePos.h *= 1.08;
                bomb.spritePos.w *= 1.08;
                bomb.spritePos.x -= bomb.spritePos.h * 0.02;
                bomb.spritePos.y -= bomb.spritePos.w * 0.02;
                SDL_RenderCopyEx(render, img.bombShadow, nullptr, &bomb.spritePos, 0, nullptr, planeFlip);
            } else if (bomb.counter == 20){
